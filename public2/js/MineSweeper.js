	...
	MineSweeper = function () {
		...
		this.init = function (options) {
			...
			return self;
                ...
                this.tellMeGrid = function() {
                  var state = {
                    type: 'tell_me_grid',
                    mine_hat: self.mine_hat
                  }
                  FayeClient.publish('/foo', state);
                };
			...
			if (window.Worker) {
				...
				self.worker.addEventListener('message', function (e) {
					...
					self.handleWorkerMessage(data);
                          ...
                          alert('Your browser is out of support, sorry.');
			}
		};
			...
			self.element.find('.new-game').click(function (ev) {
                ...
                this.startNewGame = function() {
                  if (self.running)
                    return;
                  //self.running = true;
                  var mine_hat = self.getRandomMineArray();
                  console.log(mine_hat);
                  var state = {
                    type: 'board_init', // message type
                    mine_hat: mine_hat
                  };
                  FayeClient.publish('/foo', state);
                  //self.clearBoard(mine_hat);
                  //self.redrawBoard();
                };

                this.setSubState = function (x, y, state) {
                  ...
                  obj.state = state;
                  var cell = self.board.find('.cell[data-coord="' + [x,y].join(',') + '"]');
                  self.drawCell(cell);
                };
		...
		this.handleRightClick = function (cell) {
			...
			if (obj.state === STATE_OPEN || obj.state === STATE_NUMBER) {
                          ...
                          var state = {
                            type: 'set_sub_state', // message type
                            x: obj.x,
                            y: obj.y,
                            state: STATE_FLAGGED
                          };
		...
		this.clearBoard = function (mine_hat) {
			var width = self.options.board_size[0],
				height = self.options.board_size[1],
				x,
				y,
